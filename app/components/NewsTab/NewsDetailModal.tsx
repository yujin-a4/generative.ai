    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0"}`} onClick={onClose}>
      ...
      >
        ...
        <div className="p-8 overflow-y-auto custom-scrollbar space-y-8">
          <div>
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-3 flex items-center gap-2">📝 핵심 요약</h3>
          ...
          <div className="bg-indigo-50 dark:bg-indigo-900/20 p-6 rounded-xl border border-indigo-100 dark:border-indigo-800/50">
            <h3 className="text-sm font-bold text-indigo-700 dark:text-indigo-300 uppercase mb-2 flex items-center gap-1">💡 에듀테크 Insight</h3>
            <p className="text-gray-800 dark:text-gray-200 leading-relaxed font-medium">{news.insight}</p>
          </div>

          <div className="pt-4 border-t border-gray-100 dark:border-zinc-800">
            <div className="flex flex-wrap gap-2 mb-6">
              {news.tags?.map((tag, i) => ( <span key={i} className="px-3 py-1.5 bg-gray-100 dark:bg-zinc-800 rounded-lg text-sm text-gray-600 dark:text-gray-400">{tag}</span> ))}
            </div>
            <a href={news.url} target="_blank" rel="noopener noreferrer" className="block w-full py-4 text-center bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl font-bold hover:opacity-90 transition-opacity" > 원문 기사 보러가기 → </a>
          </div>
        </div>
      </div>
    </div>
